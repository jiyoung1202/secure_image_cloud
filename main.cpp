...
int main()
{
	Mat image, image2, image3, drawing, result, image5;  //  Make images.
	...
	image = imread("C:/image/car8.jpg");  //  Load an image file.
	...
	imwrite("C:/image/car8-1.jpg",image(Rect(boundRect2[select].tl().x - 20, boundRect2[select].tl().y - 20, plate_width + 40, plate_width*0.3)));

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	///////// 이 아래는 블러링   ////////////////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	IplImage *srcImage = cvLoadImage("C:/image/car8-1.JPG", -1);
	IplImage *originalimage = cvLoadImage("C:/image/car8.JPG", -1);
	IplImage *resultImage3x3 = cvCreateImage(cvGetSize(srcImage), IPL_DEPTH_8U, 3);

	//관심영역설정
	cvSetImageROI(originalimage, Rect(boundRect2[select].tl().x - 20, boundRect2[select].tl().y - 20, plate_width + 40, plate_width*0.3));


	//cvSmooth : 이미지 스무딩(=블러링) 함수
	//cvSmooth(원본이미지, 결과이미지, 스무딩 형태, 마스크가로크기, 마스크 세로크기, 가우시안 마스크인 경우 표준편차1, 표준편차2)
	cvSmooth(srcImage, resultImage3x3, CV_BLUR, 10, 15, 0, 0);

	//이미지 합치기 함수
	cvAddWeighted(resultImage3x3, 0.3, resultImage3x3, 0.3, 0, originalimage);
	...
	cvShowImage("please", originalimage);
	//cvShowImage("5x5 image", resultImage5x5);
	//cvSetImageROI(image, Rect(boundRect2[select].tl().x - 20, boundRect2[select].tl().y - 20, plate_width + 40, plate_width*0.3));
	//cvAddWeighted(&image, 1, srcImage, 1, 0, &result);
	cvWaitKey(0);
